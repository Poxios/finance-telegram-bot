...
async def send_telegram_message(userId: int, message: str):
    bot.send_message(chat_id=userId, text=message, parse_mode="HTML")
...
def command_handler_add_stock_ticker_to_user(update, context):
    ...
    if not len(context.args):
        ...
        context.bot.send_message(chat_id=user_id, text=res_str, parse_mode="HTML")
        return
...
def command_handler_fetch_now(update, context):
    user_id = update.effective_chat.id
    context.bot.send_message(chat_id=user_id, text="Fetching finance info..")
    full_message = get_full_finance_info_message(user_id)
    context.bot.send_message(chat_id=user_id, text=full_message, parse_mode="HTML")
